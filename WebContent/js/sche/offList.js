...
function initI18n() {
    ...
    $.i18n.init({
        ...
        ns : {
            namespaces : [ 'share', 'offlineList' ],
            defaultNs : 'offlineList'
        },
        resStore : RES_LANG
    }, function() {
        $('body').i18n();
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            ...
            if(json.list.length > 0) {
                ...
                $('#schedulelist').append('<tr><td class="text-center" colspan="12">' + $.i18n.t('share:noList') + '</td></tr>');
            }
...
function popUpModal() {
	if($('input[name=listChk]:checked').length > 0) {
		...
		alertPop($.i18n.t('upPop.selectMsg'));
	}
}
...
function upload() {
	...
	jQuery.ajax({
		type: 'POST',
		url: contextPath + '/sche/off/upload.html',
		traditional: true,
		data: {
			uidArr: uidArr
		},
		success: function(data) {
			...
			try {
				...
				let msg = msgArr[msgArr.length - 1];
				
				if(msg == 'ERR') {
					alertPop($.i18n.t('share:tryAgain'));
				}else if(msg == 'DONE') {
					alertPop($.i18n.t('upPop.compUp'));
				}else {
					alertPop($.i18n.t('share:tryAgain'));
				}
			}catch(ex) {
		...
		xhrFields: {
            onprogress: function(e) {
				...
				let msgArr = res.split('@');
				let msg = msgArr[msgArr.length - 1];
				
				if(msg != 'ERR' && msg != 'DONE') {
					if(isShowProgress) {
						progressPopUpdate(msg);
					}else {
						isShowProgress = true;
						progressPop(msg);
					}
...
// 온라인 모드 변경 팝업.
function popOnlineModal() {
	if($('input[name=listChk]:checked').length > 0) {
		let listChk = document.getElementsByName('listChk');
		let isMsg2Show = false;
	
		for(let i = 0; i < listChk.length; i++) {
			if(listChk[i].checked) {
				let trialStatus = listArr[i].trialStatus;
				
				if(trialStatus == 'ONGO' || trialStatus == 'ARRIVE') {
					isMsg2Show = true;
					break;
				}
			}
		...
		if(isMsg2Show) {
			$('#onlineModalMsg2').show();
		}else {
			$('#onlineModalMsg2').hide();
		}
		
		$('#onlineModal').modal();
	}else {
		alertPop($.i18n.t('onlinePop.selectMsg'));
	}
}
...
function online() {
	...
	jQuery.ajax({
		type: 'POST',
		url: contextPath + '/sche/off/changeOn.html',
		traditional: true,
		data: {
			uidArr: uidArr
		},
			...
			try {
				...
				if(json.result) {
					if(json.error) {
						...
						alertPop($.i18n.t('onlinePop.compOnline'));
					}
				}else{
					alertPop($.i18n.t('share:tryAgain'));
				}
