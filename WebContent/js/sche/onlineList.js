...
function initI18n() {
    ...
    $.i18n.init({
        ...
        ns : {
            namespaces : [ 'share', 'onlineList' ],
            defaultNs : 'onlineList'
        },
        resStore : RES_LANG
    }, function() {
        $('body').i18n();
...
function init() {
    ...
    $('#listAllChk').click(function(){
        if($('#listAllChk').is(':checked')) {
            $('input[name=listChk]:not(:disabled)').prop('checked', true);
        }else {
...
function getScheduleDepartureList(page) {
    ...
    jQuery.ajax({
        type: 'GET',
        url: contextPath + '/sche/getScheListForOnline.html',
        data: {
            page: page,
            shiptype: shipType,
            hullnum: hullnum,
        ...
        success: function(data) {
            ...
            for(var i = 0; i < json.list.length; i++) {
				...
				if(isOff == 'Y' || trialStatus == 'ONGO' || trialStatus == 'ARRIVE') {
                ...
                text += '	<td class="">' + json.list[i].hullnum  + '</td>';
				text += '	<td class="">' + trialKey + '</td>';
                text += '	<td class="">' + json.list[i].regOwner + '</td>';
                text += '	<td class="">' + json.list[i].shiptype + '</td>';
                text += '	<td class="text-center">#' + json.list[i].projSeq + '</td>';
                ...
                text += '	<td class="text-center">' + json.list[i].sdate + '</td>';
                text += '	<td class="text-center">' + json.list[i].edate + '</td>';
                ...
                text += '	<td class="text-center">' + json.list[i].insertName + '</td>';
                text += '	<td class="text-center"><div data-toggle="tooltip" data-placement="top" title="' + insertDateTime + '">' + insertDateDate + '</div></td>';
...
function popDownModal() {
	if($('input[name=listChk]:checked').length > 0) {
		...
		alertPop($.i18n.t('downPop.selectMsg'));
	}
}
...
function download() {
	let uidArr = [];
	let listChk = document.getElementsByName('listChk');
	
	for(let i = 0; i < listChk.length; i++) {
		if(listChk[i].checked) {
			uidArr.push(listArr[i].uid);
		}
	}
	
	jQuery.ajax({
		type: 'POST',
		url: contextPath + '/sche/on/download.html',
		traditional: true,
		data: {
			uidArr: uidArr
		},
		success: function(data) {
			try {
				...
				getScheduleDepartureList(schePageNo);
				...
				if(json.result) {
					if(json.error) {
						...
						alertPop($.i18n.t('downPop.compDown'));
					}
				}else{
					alertPop($.i18n.t('share:tryAgain'));
				}
