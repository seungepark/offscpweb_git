...
function forJsonHtml(str) {console.log(str);
    return str.replace(/\n/gi,'<br/>');
}
...
function signFileAdded(input) {
	...
	if(isAddFiles) {
		$("#addSignFile").empty();
		$("#addSignFile").append('<div class="my-1">' + addFileText + '</div>');
	}
}
...
function changeUserSet() {
	...
	for(var i = 0; i < signFile.length; i++) {
		formData.append('file2', signFile[i]);
	}
...
function delSearchCookie() {console.log('ddd');
	const searchKeys = [
...
function progressPop(title) {
	$('#progressTitle').text(title);
	$('#progressPop').toast('show');
}

function progressPopUpdate(title) {
	$('#progressTitle').text(title);
}

function progressPopHide() {
	$('#progressPop').toast('hide');
	document.getElementById('progressPop').classList.remove('show');
}
